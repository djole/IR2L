...
from safety_gym_mod.envs.world import World, Robot
...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    def obs_lidar(self, positions, group):
        ...
        if self.lidar_type == 'pseudo':
            return self.obs_lidar_pseudo(positions, group)
    ...
    def obs_lidar_pseudo(self, positions, group=None):
        '''
        ...
        # Different sensor might have different range
        if group is None:
            lidar_max_dist = self.lidar_max_dist
        elif group == GROUP_GOAL:
